...
HOME_DEVPATH 			 ?= $(HOME)/homework
RAYLIB_PATH        ?= $(HOME_DEVPATH)/raylib-4.0.0
...
ifeq ($(PLATFORM),PLATFORM_WEB)
    ...
    EMSDK_PATH         ?= $(HOME_DEVPATH)/emsdk
    EMSCRIPTEN_PATH    ?= $(EMSDK_PATH)/upstream/emscripten
    CLANG_PATH          = $(EMSDK_PATH)/upstream/bin
    PYTHON_PATH         = $(EMSDK_PATH)/python/3.9.2-1_64bit
    NODE_PATH           = $(EMSDK_PATH)/node/14.15.5_64bit/bin
		export PATH         = $(shell printenv PATH);$(EMSDK_PATH);$(EMSCRIPTEN_PATH);$(CLANG_PATH);$(NODE_PATH);$(PYTHON_PATH);$$(PATH)
endif
